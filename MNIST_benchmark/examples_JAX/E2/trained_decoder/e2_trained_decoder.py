...
plt.ylabel('Fidelity')
plt.savefig(
    f'e2_trained_{num_trash_bits}_{num_data_bits}_{num_entangler_bits}_{num_layers}_trash_fidelity_training.png')
...
plt.ylabel('Fidelity')
plt.savefig(f'e2_trained_{num_trash_bits}_{num_data_bits}_{num_entangler_bits}_{num_layers}_trash_fidelity.png')
plt.show()
print("*" * 50)
...
def circuit_decoding(params, state, weights_encoder=weights_encoder):
    params = params.reshape([num_layers, -1])

    weights_encoder = weights_encoder.reshape([num_layers, -1])
...
def fid_score(params, X):
    return -jnp.mean(jax.vmap(check_fidelity, in_axes=[None, 0])(params, X))


cost_pre = fid_score(weights_decoder, X_train)
print(-cost_pre, "decoder fidelity before train - train data")
print(
    -fid_score(weights_decoder, X_test),
...
def fit(params, optimizer):
    ...
    @jax.jit
    def step(params, opt_state, X):
        loss_value, grads = jax.value_and_grad(fid_score)(params, X)
        updates, opt_state = optimizer.update(grads, opt_state, params)
        params = optax.apply_updates(params, updates)
        return params, opt_state, loss_value
    ...
    for i in range(epochs):
        ...
        train_fid_hist.append(-fid_score(params, X_train))
        test_fid_hist.append(-fid_score(params, X_test))
...
plt.xlabel('Epochs')
plt.savefig(
    f'e2_trained_{num_trash_bits}_{num_data_bits}_{num_entangler_bits}_{num_layers}_decoder_fidelity_training.png')
plt.show()
...
fid_scores_train = get_fid_scores(weights_decoder, X_train)
fid_scores_test = get_fid_scores(weights_decoder, X_test)
...
plt.legend(["Train 0", "Train 1", "Test 0", "Test 1"], loc=3)
plt.ylabel('Fidelity')
plt.savefig(f'e2_trained_{num_trash_bits}_{num_data_bits}_{num_entangler_bits}_{num_layers}_final_fidelity.png')
plt.show()
"""
fig, ax = qml.draw_mpl(circuit_decoding)(weights_decoder,  X_train[0])
fig.show()
"""
print(PARAMETERS)
